...
function PortalDropdown({ isOpen, anchorRef, children }) {
  ...
  return createPortal(
    <div className="fixed z-[9999] bg-[#1a1a1a] border border-white/10 rounded shadow-2xl py-1 max-h-48 overflow-y-auto custom-scrollbar" style={{ top: coords.top + 4, left: coords.left, width: Math.max(coords.width, 160) }} onMouseDown={(e) => e.stopPropagation()}>{children}</div>,
    document.body
  );
}
...
function LanguageSelector({ label, selected, options, onToggle, isSingle = false, showAuto = false }) {
  ...
  useEffect(() => {
    const clickOutside = (e: MouseEvent) => { if (containerRef.current && !containerRef.current.contains(e.target as Node)) setIsOpen(false); };
  ...
  return (
    <div className="relative" ref={containerRef}>
      <button onClick={(e) => { e.preventDefault(); setIsOpen(!isOpen); }} className={`flex items-center justify-between w-full px-2 py-1.5 bg-black border border-white/10 rounded text-[10px] font-mono hover:border-indigo-500/50 transition-colors ${isSingle ? 'text-blue-400' : 'text-indigo-400'}`}>
      ...
      <PortalDropdown isOpen={isOpen} anchorRef={containerRef}>
        ...
        {options.map(lang => (
          <div key={lang.id} onClick={() => { onToggle(lang.id); if (isSingle) setIsOpen(false); }} className="flex items-center justify-between px-3 py-2 hover:bg-white/5 cursor-pointer text-[10px]">
            <span className={selected.includes(lang.id) ? (isSingle ? 'text-blue-400' : 'text-indigo-400') : 'text-gray-400'}>{lang.label} ({lang.id.toUpperCase()})</span>
...
export default function VideoCard({ video, onStart, onCancel, onNavigate, globalSettings }) {
  ...
  const [srcLang, setSrcLang] = useState(subInfo.hasSubtitles && subInfo.languages[0] !== 'auto' ? [subInfo.languages[0]] : (globalSettings?.sourceLang || ['auto']));
  ...
  useEffect(() => {
    if (globalSettings) {
      ...
      if (globalSettings.workflowMode === 'force_ai') setWorkflow('whisper');
      else setWorkflow(determineBestWorkflow());
      if (!subInfo.hasSubtitles || subInfo.languages[0] === 'auto') setSrcLang(globalSettings.sourceLang || ['auto']);
    }
  }, [globalSettings]);
  ...
  if (isDir) {
    return (
      <div onClick={() => onNavigate(video.filePath)} className="group flex items-center gap-3 px-4 py-3 hover:bg-white/5 transition-colors cursor-pointer border-b border-white/5 my-1">
  ...
  const statusAccent = video.status === 'processing' ? 'border-l-indigo-500' : video.status === 'done' ? 'border-l-emerald-500' : 'border-l-transparent';
          ...
          <div className="flex items-center gap-2">
            ...
            {subInfo.hasSubtitles && (
              <div className="group/tooltip relative flex items-center gap-1 bg-indigo-500/10 border border-indigo-500/30 px-1.5 py-0.5 rounded animate-in fade-in zoom-in duration-300 cursor-help">
                ...
                <span className="text-[8px] font-bold text-indigo-400 uppercase tracking-tighter">{subInfo.subType === 'embedded' ? 'Embedded Subs' : 'SRT Found'}</span>
              </div>
            )}
          </div>
        </div>
        <div className="text-right shrink-0">
          <span className={`text-[9px] font-black uppercase tracking-widest ${video.status === 'processing' ? 'text-indigo-500 animate-pulse' : 'text-gray-700'}`}>{video.status || 'Idle'}</span>
        </div>
      </div>
      ...
      <div className="px-4 py-3 border-t border-white/5 grid grid-cols-12 gap-4 items-end">
        ...
        <div className="col-span-5 space-y-2 flex flex-col justify-between">
          ...
          <div className="flex flex-col gap-1 bg-black/40 p-1.5 rounded border border-white/5 min-h-[85px] justify-center">
            <button disabled={!subInfo.hasSubtitles} onClick={() => setWorkflow(subInfo.subType === 'embedded' ? 'embedded' : 'srt')} className={`flex items-center gap-2 px-2 py-1 text-[9px] font-bold rounded uppercase transition-colors ${(workflow === 'srt' || workflow === 'embedded') ? 'bg-indigo-500 text-white' : 'text-gray-500 hover:text-gray-300 disabled:opacity-10'}`}><LinkIcon size={10} /> {subInfo.subType === 'embedded' ? 'Use Embedded' : 'Use Matching SRT'}</button>
            <button onClick={() => setIsModalOpen(true)} className={`flex items-center gap-2 px-2 py-1 text-[9px] font-bold rounded uppercase transition-colors ${workflow === 'external' ? 'bg-indigo-500 text-white' : 'text-gray-500 hover:text-gray-300'}`}><Upload size={10} /> Import External SRT</button>
            <button onClick={() => setWorkflow('whisper')} className={`flex items-center gap-2 px-2 py-1 text-[9px] font-bold rounded uppercase transition-colors ${workflow === 'whisper' ? 'bg-indigo-500 text-white' : 'text-gray-500 hover:text-gray-300'}`}><Cpu size={10} /> AI Whisper Engine</button>
          </div>
        </div>
        ...
        <div className="col-span-4 space-y-2 flex flex-col justify-between">
          ...
          <div className="flex flex-col gap-1 bg-black/40 p-1.5 rounded border border-white/5 min-h-[85px] justify-center">
            <button onClick={() => setShowOffsets(!showOffsets)} className={`flex items-center justify-center gap-2 py-1 rounded text-[9px] font-bold uppercase transition-all ${showOffsets ? 'bg-indigo-500/20 border border-indigo-500/50 text-indigo-400' : 'text-gray-500 hover:text-gray-300'}`}><Clock size={11} /> Sync Offset</button>
            {video.status === 'processing' ? (
              <button onClick={() => onCancel(video.id)} className="w-full py-1.5 bg-red-500/20 text-red-500 rounded font-bold text-[9px] uppercase hover:bg-red-500 hover:text-white transition-colors border border-red-500/30">Terminate</button>
            ) : (
              <button onClick={() => onStart(video.id)} className="w-full py-1.5 bg-indigo-600 text-white rounded font-bold text-[9px] uppercase shadow-lg hover:bg-indigo-500 active:scale-95 transition-all">Execute Job</button>
            )}
          </div>
        </div>
      </div>
