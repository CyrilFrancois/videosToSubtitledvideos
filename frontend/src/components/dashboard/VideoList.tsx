...
export default function VideoList({ videos, isNested = false }: VideoListProps) {
  ...
  const handleFolderToggle = (item: VideoFile) => {
    ...
    if (newExpanded.has(item.id)) {
      ...
      // This call now works because we added 'navigate' to page.tsx
      actions.navigate(item.filePath); 
    }
    setExpandedFolders(newExpanded);
  };

  // 1. LOADING STATE (Only shown at root level during initial scan)
  ...
  // 3. MAIN RENDER
  // We only wrap the root level in a section and max-width container
  const content = (
    <div className={`space-y-1 ${!isNested ? "max-w-5xl mx-auto pb-20" : ""}`}>
      {videos.map((item) => (
        <div key={item.id} className="flex flex-col">
          ...
          {item.is_directory && expandedFolders.has(item.id) && item.children && (
             <div className="ml-[2.45rem] border-l border-white/[0.05] pl-2 animate-in slide-in-from-left-2 duration-200">
  ...
  return isNested ? content : (
    <section className="flex-1 overflow-y-auto custom-scrollbar px-6 pt-4">
      {content}
    </section>
  );
}