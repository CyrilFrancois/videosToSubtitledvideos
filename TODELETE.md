...
This architecture centers on an Asynchronous Task Orchestrator in the backend. Since media processing is resource-heavy, the frontend shouldn't just "wait" for a response; it triggers a persistent queue and listens to a real-time status feed.1. The Enhanced Data PipelineWhen the user clicks "Execute," the frontend sends a batch of job configurations to backend/main.py. The backend then processes each file through this specific pipeline:Phase 0: Transcription (Only if needed)Workflow: whisper or force_ai.Service: transcriber.py extracts audio and runs local inference.Result: A raw .srt file. Whisper often makes phonetic errors (e.g., confusing "Wait for Barry" with "Wait for Bury") and struggles with character-specific gender.Phase 1: Context Injection & Refining (The "Sanity Check")Condition: Triggered only when Whisper is used.Service: translator.py.Action: The backend sends the filename to GPT to generate a "Context Profile" (Summary, Characters, Tone).Refinement: The LLM takes the Raw Whisper SRT + Context Profile and "re-writes" the transcript. It fixes phonetic hallucinations and ensures character names/genders match the story context.Phase 2: TranslationService: translator.py.Action: Uses the refined transcript and the Context Profile to translate into all targetLanguages. The context ensures grammatical accuracy (like gender agreement in French/Spanish) that a blind translation would miss.Phase 3: Muxing & CleanupService: muxer.py.Action: Combines the video with the new refined/translated tracks into a final .mkv.Cleanup: If cleanUp is enabled, the original source is deleted only after the new file is verified.2. Real-Time State ManagementThe backend communicates the status of every file in the batch via Server-Sent Events (SSE). These states are reflected in the VideoCard (Execute area) and the GlobalProgress header.StateDescriptionQueuedWaiting for its turn in the linear queue.TranscribingLocal Whisper is running on the audio.ContextualizingGPT is researching the story context from the filename.RefiningGPT is correcting Whisper's errors using the context.TranslatingGenerating the target language SRTs.MuxingFFmpeg/mkvmerge is building the final container.CompletedJob successful, artifacts removed.InterruptedState of the active file when the "Kill Switch" was hit.CancelledState of all queued files when the batch was aborted.3. The "Kill Switch" & Rollback ArchitectureCancellation is a global stop command rather than a per-file toggle.The Trigger: User clicks "Cancel" in the UI.Queue Purge: main.py immediately empties the list of pending jobs, marking them as Cancelled.Process Assassination: The backend identifies the PID (Process ID) of the active subprocess (Whisper, FFmpeg, or the LLM loop) and kills it.Artifact Rollback: The orchestrator enters a cleanup hook. It identifies the active file's temporary artifacts (e.g., temp.wav, raw_whisper.srt, or a half-written .mkv) and deletes them.State Broadcast: A final SSE signal is sent so the frontend knows exactly which file was Interrupted and which were Cancelled.
